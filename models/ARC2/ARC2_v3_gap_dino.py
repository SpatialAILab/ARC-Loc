...
class CVM(nn.Module):
    def __init__(self, device, grd_enc, sat_enc, grd_bev_res, grd_height_res, sat_bev_res, grid_size_h, grid_size_v,
    ...
    def forward(self, grd, sat, other=None):
        ...
        ## =============== Feature Extraction ===============
        with torch.no_grad():
            # with torch.amp.autocast('cuda', dtype=torch.bfloat16): # 연산 가속
            grd_feature = self.grd_encoder(grd)
            sat_feature = self.sat_encoder(sat) # dino 
